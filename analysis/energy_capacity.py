...
def calc_energy_capacity(uptime, power_capacity, rte, region, month, signal="cost"):
    """
    ...
    optimized_load = idxparam_value(flex.model.net_facility_load)
    baseload = np.ones_like(data)

    normalized_energy_capacity = np.sum(np.clip(baseload - optimized_load, 0, None)) / len(data)  # MWh per hour

    return normalized_energy_capacity * 100 # conver to pct 